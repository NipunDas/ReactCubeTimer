...
export const SessionSelect: React.FunctionComponent = (): JSX.Element => {
    const { sessionList, currentIndex, setCurrent, createSession, deleteSession } = useContext(SessionContext)
            ...
            <Grid size={6}>
                <Select value={currentIndex.toString()} onChange={handleSelect}> 
                    {sessionList.map((session: Session, index: number) => (
                        <MenuItem key={index} value={index.toString()}>{session.name}</MenuItem>
                    ))}
                    <MenuItem key={-1} value={'new'}>New</MenuItem>
                    {sessionList.length >= 2 ? <MenuItem key={-2} value={'delete'}>Delete</MenuItem> : null}
                </Select>
            </Grid>
        </Grid>
